...
class CurrentWeatherCollectionView: UICollectionView {
  ...
  init(cellType: CollectionViewCellCellTypes) {
    ...
    case .hourlyCollectionViewCell:
      ...
      layout.itemSize = CGSize(width: adapted(dimensionSize: 72, to: .width),
                               height: adapted(dimensionSize: 104, to: .height))
      layout.scrollDirection = .horizontal
      let frame = CGRect(x: adapted(dimensionSize: 0, to: .width),
                         y: adapted(dimensionSize: 16, to: .height),
                         width: adapted(dimensionSize: 390, to: .width),
                         height: adapted(dimensionSize: 104, to: .height))
      super.init(frame: frame, collectionViewLayout: layout)
    ...
    case .weatherCollectionViewCell:
      let layout: UICollectionViewFlowLayout = UICollectionViewFlowLayout()
      layout.sectionInset = UIEdgeInsets(top: adapted(dimensionSize: 17, to: .height),
                                         left: adapted(dimensionSize: 22, to: .width),
                                         bottom: adapted(dimensionSize: 17, to: .height),
                                         right: adapted(dimensionSize: 22, to: .width))
